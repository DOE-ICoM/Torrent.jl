...
function open_precipitation_series(
  ...
  for i in start_index:end_index
    ...
    (_, evt) = time_computation("") do 
      ...
      update!(prog, i-start_index; showvalues = [("file", short_filename(filename)), ("sources", length(p.sources)), ("flux", p.total_flux)])
...
function open_multiband_precipitation(
  ...
  upper = if max_index>0 max_index else length(iter) end
